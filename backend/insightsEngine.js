import { calculateCycleState, getPhaseDetails } from './cycleCalculator.js';
...
export function generateWellnessSuggestions(cycleState, logs, profile) {
  ...
  suggestions.push({
    ...
    title: '🏃‍♀️ Sahi Exercise Target',
    description: cycleState.phase === 'Menstrual' 
      ? 'Aaj heavy workout avoid karein — sirf light walking aur gentle stretching karein.' 
      : cycleState.phase === 'Follicular' 
      ? 'Aapki body energy high hai — aaj stamina building & strength training best rahegi.'
      : cycleState.phase === 'Ovulatory'
      ? 'Peak stamina day! Cardio, swimming ya HIIT exercise ke liye best time hai.'
      : 'Light Pilates aur evening walk karein. Heavy lifting reduce karein.',
  });
  ...
  suggestions.push({
    ...
    description: cycleState.phase === 'Menstrual' 
      ? 'Garam paani, ginger tea, spinach aur iron-rich khana khaayein.' 
      : cycleState.phase === 'Follicular' 
      ? 'Fresh salads, curd, aur high-protein diet lein taaki energy bani rahe.'
      : cycleState.phase === 'Ovulatory'
      ? 'Fresh fruits (berries, bananas) aur nuts khaayein to stay active.'
      : 'Complex carbs (oats, daliya) aur dark chocolate lein, spicy/salty food kam karein.',
  });
  ...
  if (recentSymptoms.includes('bloating')) {
    suggestions.push({
      icon: 'spa',
      title: '🎈 Pet Phoolna (Bloating) Relief',
      description: 'Pudina (Peppermint) tea piyein aur khane me namak (salt) kam karein.',
    });
  }
  ...
  if (recentSymptoms.includes('cramps')) {
    suggestions.push({
      ...
      title: '⚡ Cramps Relief Desi Nuskha',
      description: 'Lower belly par hot water bag rakhein aur ajwain-warm water piyein.',
    });
  }
  ...
  if (avgSleep < 7) {
    suggestions.push({
      ...
      description: 'Raat ko sone se 30 min pehle phone rakh dein aur 8 ghante ki sleep complete karein.',
    });
  }
